// traducir-locize.cjs
// Script para traducir claves locales de LuxScaler usando Gemini y subir el resultado a Locize
...
// ==================== CLAVES MAESTRAS (FUENTE DE VERDAD) ====================
...
// ==================== TRADUCIR ====================
async function traducir(texto, idioma, clave) {
    if (!texto || texto.trim().length === 0) return texto;

    const prompt = `Traduce al ${IDIOMAS[idioma]} para una app de escalado de imágenes con IA.
Clave: "${clave}"

REGLAS:
- NO traduzcas: PhotoScaler™, LuxScaler, API, CDN, tokens, vs, ZOOM, STUDIO, 4K, 3D... (términos técnicos o marcas)
- Mantén variables: {{variable}}
- Mantén HTML si hay
...
Traducción:`;
    ...
    try {
        ...
        console.error(`Error traduciendo "${texto}" a ${idioma}:`, error.message);
        return texto;
    }
}

// ==================== SUBIR A LOCIZE ====================
async function subirLocize(idioma, traducciones) {
    const url = `https://api.locize.app/update/${LOCIZE_PROJECT_ID}/${LOCIZE_VERSION}/${idioma}/${LOCIZE_NAMESPACE}`;
    ...
    return new Promise((resolve, reject) => {
        ...
        req.on('error', reject);
        req.write(JSON.stringify(traducciones));
        req.end();
    });
}
...
// ==================== MAIN ====================
async function main() {
    console.log('🚀 Traduciendo LuxScaler con Gemini (Basado en Claves Locales)...\n');

    const total = Object.keys(enTranslations).length;
    console.log(`✅ ${total} claves encontradas localmente.\n`);

    // 1. Primero aseguramos que el inglés esté en Locize
    console.log('📤 Asegurando inglés en Locize...');
    try {
        await subirLocize('en', enTranslations);
        console.log('✅ Inglés sincronizado.\n');
    } catch (e) {
        console.warn('⚠️ Nota: Sync de inglés devolvió error (posiblemente 412), continuando con traducciones...');
    }

    // 2. Traducir cada idioma
    for (const [codigo, nombre] of Object.entries(IDIOMAS)) {
        console.log(`🌍 Traduciendo a ${nombre} (${codigo})...`);
        const traducciones = {};

        let i = 0;
        for (const [clave, texto] of Object.entries(enTranslations)) {
            i++;
            traducciones[clave] = await traducir(texto, codigo, clave);

            if (i % 10 === 0) {
                process.stdout.write(`.`);
            }

            // Delay para rate limiting (Gemini Free tier ~15 RPM, Paid ~2000 RPM)
            // Usamos 200ms por request para ~300 RPM.
            await new Promise(r => setTimeout(r, 200));
        }
        console.log(`\n✅ ${codigo}: ${total} traducciones completas`);

        // 3. Subir a Locize
        console.log(`📤 Subiendo ${codigo} a Locize...`);
        try {
            await subirLocize(codigo, traducciones);
            console.log(`✅ ${codigo}: Sincronizado.\n`);
        } catch (e) {
            console.error(`❌ Error subiendo ${codigo}:`, e.message);
        }
    }
