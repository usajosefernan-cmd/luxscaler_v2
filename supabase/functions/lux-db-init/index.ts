...
serve(async (req: Request) => {
    ...
    try {
        ...
        console.log("[LUX-DB-INIT] Executing SCHEMA UPDATES...");

        await sql.unsafe(COMPLETE_SEED_SQL);

        console.log("[LUX-DB-INIT] SUCCESS - Projects/Documents Ready");

        await sql.end();

        return new Response(
            JSON.stringify({ success: true, message: "DB INIT SUCCESS: projects and documents tables ready" }),
            { headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
    } catch (error) {
        console.error("[LUX-DB-INIT] ERROR:", error.message);
        return new Response(
